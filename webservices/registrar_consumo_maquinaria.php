    ...
    }else
    {
      ...
      if( $con ) //Conexion establecida
      {
        ...
        $query = "INSERT INTO consumo_maquinaria(cod_maquinaria, litros, fecha_hora)
                  VALUES('". $_REQUEST["cod_maquinaria"] ."','". $_REQUEST["litros"] ."','". $fecha->format("Y-m-d H:i:s") ."')";
        if( mysqli_query($con,$query) )
        {
          ...
          // $cc = null;
          $subject = "TIERRA DULCE: Registro de consumo de combustible  [". $fecha->format("Y-m-d H:i:s") ."]";
          $msg = '<html>
                    <body>
                      <p>
                        ...
                        Galones:  <b>'. round( ( intval($_REQUEST["litros"] ) * 0.264172), 2) .'</b></br>
