    ...
    switch ( $_POST["opcion"])
    {
      ...
      case '2': //Actualizar
          if( empty($_POST["descripcion"]) )
          {
            ...
            $query = "UPDATE unidades_medida SET descripcion=? WHERE id_unidades_medida=?";
            $st = $con->prepare($query); //Statement
            if( $st )
            {
              $desc = utf8_decode( $_POST["descripcion"] );
              $id = $_POST["id_unidades_medida"];
      ...
      case '3':
          //Borrar
          $query = "DELETE FROM unidades_medida WHERE id_unidades_medida='". $_POST["id_unidades_medida"] ."'";
          $resultado = mysqli_query($con, $query);
          if( $resultado ) //Se realizó el registro
          {
            $retorno["estado"] = "OK";
      ...
      case '4':
          ...
          {
            $medidas[] = array(
                                  "id_unidades_medida" => $row["id_unidades_medida"],
      ...
      case '5':
        //Editar campo
        $query = "UPDATE unidades_medida SET ". $_POST["campo_query"] ."='". $_POST["valor_query"] ."' WHERE id_unidades_medida='". $_POST["id"] ."'";
