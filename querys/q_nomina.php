  ...
  if( $con )
  {
    switch ( $_POST["opcion"])
    {
      case 1: //Insertar
        ...
        break;
      case 2: //Actualizar
          if( empty($_POST["fecha"]) ||  empty($_POST["valor"]) ){
            $retorno["estado"] = "ERROR";
            $retorno["msg"] = "Parámetros incompletos";
          }
          else{
            $query = " UPDATE mvto_nomina SET   fecha ='". $_POST["fecha"] ."',
                                                valor= '". $_POST["valor"] ."',
                                                descripcion='". utf8_decode($_POST["descripcion"]) ."' 
                                          WHERE id_mvto_nomina='". $_POST["id"] ."' ";
            $resultado = mysqli_query($con, $query);
            if( $resultado ) {
              $retorno["estado"] = "OK";
            }
      ...
      case 3:
          //Borrar
          $query = "DELETE FROM mvto_nomina WHERE id_mvto_nomina='". $_POST["id"] ."'";
          $resultado = mysqli_query($con, $query);
          if( $resultado ) //Se realizó el registro
          {
            $retorno["estado"] = "OK";
      ...
      case 4:
          ...
          while( $row = mysqli_fetch_array( $resultado ) ){
            $nomina[] = array(
                              "id" => $row["id_mvto_nomina"],
                              "fecha" => $row["fecha"],
                              "valor" => $row["valor"],
                              "descripcion" => utf8_encode($row["descripcion"]),
                              "empleado" => $row["personal_id_personal"],
                              "nEmpleado" => utf8_encode($row["nombre_personal"]),
                            );
          }
